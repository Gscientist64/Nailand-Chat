  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (currentStep === 1) {
      ...
      try {
        ...
        const directResponse = await fetch('/api/v1/auth/signup', {
          method: 'POST',
          headers: {
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-2 mb-3">
              <button 
                type="button"
                onClick={() => googleLogin()}
                className="w-full flex items-center justify-center gap-2 border border-gray-300 rounded-lg py-2.5 px-3 hover:bg-gray-50 transition text-sm"
>
